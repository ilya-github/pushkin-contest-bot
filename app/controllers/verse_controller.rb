 ...
 def str_rev(str)
  ...
  def v_post
    ...
    result = Verse.where( 'text LIKE ?', '%'+term1+'%' )
     ...
     # word_one_copy[t] = "_"
      word_one_copy= word_one.gsub(word_one[t],'_')
      result = Verse.find_by( 'text2 LIKE ?', '%'+word_one_copy+'%' )
      unless result.blank?
        id_verse = result.id
        position_one = word_one_copy.gsub("_","(\w+|[а-яА-Я]+)")
        result.text2.split("\r\n").each do |e|
          unless e.scan(/#{position_one}/).blank? then break end
    ...
    if result.text2.split("\r\n").count == count then count -=1 end
